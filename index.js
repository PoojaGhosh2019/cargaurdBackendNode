...
// Middleware for POST /events/handled endpoint
async function deleteNest(req, res, next) {
    const keyToDelete = Object.values(req.body);
    console.log("key to delete ",keyToDelete)
    
    //update came from client, the event with id processed, delete it from nest
    nests = nests.filter(n => 
        //console.log(n.id)
        n.id !== keyToDelete[0]);
    console.log("after deleting the event ",nests)

    res.json(keyToDelete)
}
...
async function addNest(req, res, next) {
  ...
  //console.log(Object.keys(newNest))
  nests.push(newNest);
  console.log("adding event in nest ",nests)
...
app.post('/events/handled',deleteNest);
